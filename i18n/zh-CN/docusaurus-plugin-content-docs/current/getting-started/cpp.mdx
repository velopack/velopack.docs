...
2. Configure Unicode Support:
   #### Windows
   On Windows, to enable unicode support for this library you **_must_** 
   ...
   manifest. Failure to do so will potentially result in your application **_failing to update_** if there are any unicode characters in your user's 
   ...
   - If you are not using Qt, you'll need to [install ICU4C](https://icu.unicode.org/). Like many other C libraries, 
     installing the development package with your package manager (`apt`, `brew` etc) _should_ be sufficient to make it available to compilers.
...
8. Package your Velopack release / installers:
   ```sh
