...
To integrate Velopack into your application, you _must_ initialise the Velopack as early as possible in app startup, and you _should_ add update checking code somewhere.
...
:::warning
Since `current` is replaced with the new version during updates, it's not safe to store settings, logs, etc in the `current` dir where your app lives. See **_[Preserving Files](./preserved-files.mdx)_** for more info.
:::
