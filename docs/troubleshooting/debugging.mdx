...
Velopack reads a handful of environment variables at runtime. These affect the behavior of the library and the bundled binaries (Setup.exe / Update.exe), and are unrelated to the `vpk` build tool's command-line options.

| Variable | Set by | Description |
|---|---|---|
| `VELOPACK_FIRSTRUN` | Velopack | Set when your app is launched for the very first time after install. Velopack reads and clears it inside `VelopackApp.Run()`, surfacing it via the `OnFirstRun` hook. Read it only through the hook, not directly. |
| `VELOPACK_RESTART` | Velopack | Set when your app was restarted by Velopack to apply an update. Velopack reads and clears it inside `VelopackApp.Run()`, surfacing it via the `OnRestarted` hook. |
| `VELOPACK_TEMP` | You | Overrides the temp/working directory Velopack uses for downloading and staging updates. Useful in locked-down or CI environments where the default temp location is unavailable. |
| `VELOPACK_DEBUG` | You | When set, Velopack does **not** automatically exit the process after running a fast-callback hook (install/update/uninstall). Useful for attaching a debugger to inspect hook behavior. Leave unset in production. |
