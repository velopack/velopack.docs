...
3. Invokes the Velopack CLI to package your application. `-u` (alias of `--packId`) sets your unique app id, `-v`
(alias of `--packVersion`) calls upon the `version` variable assigned earlier, which is accessed using the `id` of the
step that assigned it (`get-version`), and `-p` (alias of `--packDir`) is pointed at the `publish` directory that was used
in the previous step. For more information on the Velopack CLI and which flags are available for the `pack` command,
[see here](../packaging/overview.mdx).
...
Alternatively, you can run `vpk` without installing it globally using the [`dnx`](https://learn.microsoft.com/dotnet/core/tools/dotnet-tool-exec?WT.mc_id=DT-MVP-5003472) command. Here `--version <vpk-version>` selects which version of the `vpk` **tool** to run (this is unrelated to `-v`, which sets your **application's** version). Make sure to use the same `vpk` version as the Velopack package referenced in your application:
...
Replace `<vpk-version>` (e.g. `1.0.0`) with the version of the Velopack package you're using in your application.
