...
When `AllowVersionDowngrade` is enabled, the auto-apply-on-startup behavior will not apply the lower-or-equal version for you, so you must call an explicit apply method. See [Targeting a specific version](./specific-version.mdx) for the full details.
