...
:::info
This is the low-level C ABI (`vpkc_*` functions, declared in `Velopack.h`), suitable for C projects and for calling from other languages via FFI / p/invoke. Most C++ projects should prefer the idiomatic C++ classes documented on the other pages.
:::
...
extern "C"
{
    ...
    vpkc_update_source_t* vpkc_new_source_gitea(char const* psz_repo_url, char const* psz_access_token, bool b_prerelease);

    vpkc_update_source_t* vpkc_new_source_velopack_flow(char const* psz_base_uri);
...
#### Member variables
  ...
  - `DeltasToTargetCount` - The number of elements in the DeltasToTarget array.
...
- **`psz_file_path`** — The path to a local directory containing updates.
...
- **`psz_repo_url`** — The GitHub repository URL (e.g. “https://github.com/user/repo”).
- **`psz_access_token`** — Optional access token for private repositories (can be null).
- **`b_prerelease`** — Whether to include pre-release versions.
- **Returns** — A new vpkc\_update\_source\_t instance, or null on error.
...
- **`psz_repo_url`** — The GitLab repository URL (e.g. “https://gitlab.com/user/repo”).
- **`psz_access_token`** — Optional access token for private repositories (can be null).
- **`b_prerelease`** — Whether to include pre-release versions.
- **Returns** — A new vpkc\_update\_source\_t instance, or null on error.
...
- **`psz_repo_url`** — The Gitea repository URL (e.g. “https://gitea.example.com/user/repo”).
- **`psz_access_token`** — Optional access token for private repositories (can be null).
- **`b_prerelease`** — Whether to include pre-release versions.
- **Returns** — A new vpkc\_update\_source\_t instance, or null on error.
...
- **`psz_base_uri`** — Optional base URI for the Velopack Flow API (can be null for default).
- **Returns** — A new vpkc\_update\_source\_t instance, or null on error.
...
- **`progress_callback_id`** — The ID of the progress callback to send the update to.
- **`progress`** — The progress value to send (0-100).
...
- **`psz_url_or_path`** — Location of the http update server url or path to the local update directory.
- **`p_options`** — Optional extra configuration for update manager.
...
- **`p_source`** — A pointer to a custom UpdateSource.
- **`p_options`** — Optional extra configuration for update manager.
- **`p_locator`** — Optional explicit path configuration for Velopack. If null, the default locator will be used.
- **`p_manager`** — A pointer to where the new vpkc\_update\_manager\_t\* instance will be stored.
- **Returns** — True if the update manager was created successfully, false otherwise. If false, the error will be available via `vpkc_get_last_error`.
...
- **`p_manager`** — The update manager instance.
- **`psz_version`** — A buffer to store the version string.
- **`c_version`** — The size of the `psz_version` buffer.
...
- **`p_manager`** — The update manager instance.
- **`psz_id`** — A buffer to store the app id string.
- **`c_id`** — The size of the `psz_id` buffer.
...
- **`p_manager`** — The update manager instance.
- **`p_update`** — A pointer to where the new vpkc\_update\_info\_t\* instance will be stored if an update is available.
...
- **`p_manager`** — The update manager instance to free.
...
- **`p_update_info`** — The update info instance to free.
...
- **`p_asset`** — The asset instance to free.
...
- **`b_auto_apply`** — True to automatically apply updates, false otherwise.
...
- **`c_args`** — The number of arguments in `p_args`.
...
- **`p_locator`** — The locator configuration to use.
...
- **`cb_first_run`** — The callback to run on first run. The callback takes a user data pointer and the version of the app as a string.
...
- **`cb_restarted`** — The callback to run after the app is restarted. The callback takes a user data pointer and the version of the app as a string.
...
- **`psz_error`** — A buffer to store the error message.
- **`c_error`** — The size of the `psz_error` buffer.
...
- **`cb_log`** — The callback to call with log messages. The callback takes a user data pointer, a log level, and the log message as a string.
- **`p_user_data`** — Optional user data to be passed to the callback.
