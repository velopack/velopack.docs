...
If you were to provide the RID `--runtime win10-arm64`, any users trying to install your app on Windows 7, 8, or 8.1 will receive a message saying their operating system is 
not supported. Similarly, if a Windows 11 user with an x64 cpu were trying to install - it would also fail with a helpful message that their CPU is not supported.
...
:::tip
Runtimes indicate what version and architecture your application supports, but does not enable _cross compiling_ which allows you to compile releases 
for a OS different for the one you are using. [See more about cross compiling here](./cross-compiling.mdx).
:::
