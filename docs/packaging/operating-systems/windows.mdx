...
The root-level `YourApp.exe` is a small execution stub which launches the real executable inside `current`. It is created by both `Setup.exe` and the [`.msi`](../installer.mdx#windows-msi-installer) installer, so shortcuts/launchers can point at a stable path that survives updates.
...
- A process with its CWD (current working dir) inside that folder (for example, a `cmd.exe` process).
...
:::info
The `Setup.exe` installer is per-user and installs to `%LocalAppData%\{packId}`, so it does not require elevation or use privileged directories. If you need a per-machine install to `Program Files`, use the [MSI installer](../installer.mdx#windows-msi-installer) with `--msi --instLocation PerMachine`, which installs to `Program Files\{publisher}\{packTitle}` (HKLM registry, requires elevation).
:::
