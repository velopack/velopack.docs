...
The following Linux-specific options are available when Velopack generates the bundle for you:
- `--categories {names}` — the [freedesktop.org Desktop Menu](https://specifications.freedesktop.org/menu-spec/latest/category-registry.html) categories written into the `.desktop` file. Defaults to `Utility`.
- `--compression {algo}` — the compression algorithm used for the AppImage, either `gzip` (the default) or `xz`. `xz` produces smaller files at the cost of slower (de)compression.
...
## What is an AppImage?
AppImage does not install software in the traditional sense (i.e., it does not put files all over the place in the system).

It uses one file per application. Each one is self-contained: it includes all libraries the application depends on and that are not part of the base system. 
In this regard, it is similar to "application virtualization". 
One can use an AppImage file even without being a superuser, or while using a live CD. 
