...
YMMV, for some applications this may give you big size savings, for others it may not. Either way it will be a lot slower, about on-par with the `bsdiff` algorithm.

The `--delta` option accepts three modes: `None` (disable delta generation), `BestSpeed` (the default), and `BestSize`.
...
## Generating and applying deltas manually
Normally deltas are created and applied for you automatically, but the `vpk delta` command group lets you work with them directly:

- `vpk delta generate --base {old}.nupkg --new {new}.nupkg --output {patch}` — creates a delta patch from two full `.nupkg` releases. Like `vpk pack`, it accepts a `--mode` of `None`, `BestSpeed` (default), or `BestSize`.
- `vpk delta patch --base {old}.nupkg --patch {patch} --output {new}.nupkg` — reconstructs a full package from a base package plus one or more patches. `--patch` / `-p` may be specified multiple times to apply patches in sequence.