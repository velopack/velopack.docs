...
The above will disable the `signParallel` argument and only sign one file at a time. If your signing tool supports batch signing multiple files at a time, you can specify `{{file...}}` instead. 

:::tip
You should always pass absolute path to your custom signing binary, and for any other files in your signing arguments, as there is no guarentee that signtool will be run in the same working directory as vpk.
:::
