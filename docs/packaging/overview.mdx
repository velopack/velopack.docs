...
- `--mainExe {exeName}` The main executable to be started after install, and the binary that will [handle Velopack Hooks](../integrating/overview.mdx). This is primarily a Windows concept — on macOS the entry point comes from the `.app` bundle's `Info.plist`, and on Linux from the generated `.desktop` file.
...
- `--exclude {regex}` A regex of files to exclude from the package, in addition to the built-in defaults. By default `.pdb` files are excluded (the default value is `.*\.pdb`), so if you want to ship `.pdb` files you must override this.

:::tip
Any `vpk` option can also be supplied via an environment variable named `VPK_<OPTION>` (the option name upper-cased with underscores, eg. `--keyId` becomes `VPK_KEY_ID`). This is the recommended way to pass secrets in CI without exposing them on the command line.
:::
