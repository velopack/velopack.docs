...
{!props.showMultipleTfmSuggestion && (
    ...
    <p>
    Replace <code>-r win-x64</code> with the <a href="https://learn.microsoft.com/dotnet/core/rid-catalog">runtime identifier (RID)</a> for your target platform, such as <code>osx-arm64</code> or <code>linux-x64</code>.
    </p>
    </>
)}
...
<Admonition type="tip">
    Starting with .NET 7, the `-o`/`--output` option [can no longer be used with a solution file](https://learn.microsoft.com/en-us/dotnet/core/compatibility/sdk/7.0/solution-level-output-no-longer-valid).
